        ...
        for i in range(min(batch_size, file_count)):
            ...
            for token in token_hashmap:
                if token in inverted_idx.keys():
                    inverted_idx[token].append((file_index, token_hashmap[token], idx_token_hashmap[token]))
                else:
                    inverted_idx[token] = [(file_index, token_hashmap[token], idx_token_hashmap[token])]

            file_obj.close()
            print("Done with " + file_dict["url"] + " : " + str(i))
            file_index += 1

        file_name = "jsonData_" + str(pickle_id) + ".json"
